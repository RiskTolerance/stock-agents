...
const fundamentalStep = createStep({
  ...
  execute: async ({ inputData }) => {
    const result = await fundamentalAgent.stream([
      { role: 'user', content: `Analyze the fundamentals for stock symbol: ${inputData.symbol}` }
    ]);
    let output = '';
    for await (const chunk of result.textStream) {
      output += chunk;
    }
    return { ...inputData, fundamental: output };
  },
});
...
const sentimentStep = createStep({
  ...
  execute: async ({ inputData }) => {
    const result = await sentimentAgent.stream([
      { role: 'user', content: `Analyze the sentiment for stock symbol: ${inputData.symbol}` }
    ]);
    let output = '';
    for await (const chunk of result.textStream) {
      output += chunk;
...
const newsStep = createStep({
  ...
  execute: async ({ inputData }) => {
    const result = await newsAgent.stream([
      { role: 'user', content: `Analyze the news for stock symbol: ${inputData.symbol}` }
    ]);
    let output = '';
    for await (const chunk of result.textStream) {
      output += chunk;
...
const technicalStep = createStep({
  ...
  execute: async ({ inputData }) => {
    const result = await technicalAgent.stream([
      { role: 'user', content: `Analyze the technicals for stock symbol: ${inputData.symbol}` }
    ]);
    let output = '';
    for await (const chunk of result.textStream) {
      output += chunk;
...
const bullishStep = createStep({
  ...
  execute: async ({ inputData }) => {
    const result = await bullishAgent.stream([
      { role: 'user', content: `Given the following context, argue the bull case for ${inputData.symbol}:\n${JSON.stringify(inputData)}` }
    ]);
    let output = '';
    for await (const chunk of result.textStream) {
      output += chunk;
...
const bearishStep = createStep({
  ...
  execute: async ({ inputData }) => {
    const result = await bearishAgent.stream([
      { role: 'user', content: `Given the following context, argue the bear case for ${inputData.symbol}:\n${JSON.stringify(inputData)}` }
    ]);
    let output = '';
    for await (const chunk of result.textStream) {
      output += chunk;
...
const riskStep = createStep({
  ...
  execute: async ({ inputData }) => {
    const result = await riskAgent.stream([
      { role: 'user', content: `Analyze the risk for stock symbol: ${inputData.symbol}` }
    ]);
    let output = '';
    for await (const chunk of result.textStream) {
      output += chunk;
...
const traderStep = createStep({
  ...
  execute: async ({ inputData }) => {
    const result = await decisionAgent.stream([
      { role: 'user', content: `Given the following context, make a final trading decision (BUY, SELL, HOLD) for ${inputData.symbol}:\n${JSON.stringify(inputData)}` }
    ]);
    let output = '';
    for await (const chunk of result.textStream) {
      output += chunk;
